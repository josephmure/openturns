  ...
  , domain_(domain)
{
  ...
  const Sample dummySample(1, domainDimension);
  const Sample trySample(domain_.computeDistanceToDomain(dummySample)); // throws if Domain does not know how to compute the distance
  EvaluationImplementation::setInputDescription(dummySample.getDescription());
} // DistanceToDomainEvaluation
...
/* Comparison operator */
Bool DistanceToDomainEvaluation::operator ==(const DistanceToDomainEvaluation & ) const
{
  return true;
}
...
{
  Point result(1);
  result[0] = domain_.computeDistanceToDomain(inP);
...
Sample DistanceToDomainEvaluation::operator() (const Sample & inSample) const
{
  ...
  return domain_.computeDistanceToDomain(inSample);
}
